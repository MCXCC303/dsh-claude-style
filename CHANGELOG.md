...
### Fixed
- **文件引用（行内文件名）颜色与超链接不一致**：聊天里的 `` `CHANGELOG.md` `` 这类**文件引用**是宿主把行内代码里的文件路径解析成按钮（`.fileMention`）后的产物，但皮肤的通配行内代码规则（`code:not(pre code)`，特异性 (0,2,1)）压过宿主自己的链接色类（(0,1,0)），于是同一段话里文件引用是代码片的暖红、真正的超链接是链接蓝。现在文件引用与超链接共用一套材质：链接蓝文字（亮 `#184F95` / 暗 `#8AB4F8`）、同字重 500，代码片底纹与描边改用该蓝的淡色（与引用块同一组 token），hover/focus 保持宿主原本的点线下划线。静止态不加下划线——等宽字体在代码片里下划线会与字形下伸部和底边打架，宿主自己也是只在 hover 时画。普通行内代码仍是暖红代码片，不受影响；宿主类名带哈希，故按仓库纪律取最长稳定片段 `[class*="_fileMention"]`。
