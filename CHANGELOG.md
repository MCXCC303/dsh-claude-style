...
### Added
- **Grok 行也接上厂商字标**：`src/assets/icons/wordmarks/grok.svg` 取自 Lobe Icons 的 `grok-text.svg`（同一钉住版本 `@lobehub/icons-static-svg@1.95.0`，与 `fetch-lobe-icons.mjs` 同一套规范化：去掉 `<title>`/`style`/em 宽高），viewBox 裁到墨迹 `2 2 58.598 20`。它的「G」是圆形字母、上下各有出格（墨迹 2..22，平底基线在 21.645），所以这一枚裁到整个墨迹而不是大写带：盒子底边比基线低 0.355 单位，13px 下约 0.17px，肉眼不可见，换来的是既不必依赖 overflow、也不会切到「G」。宽度 27.42px 对文字「Grok」的 30.5px，略窄，不会撑宽行。
- **DeepSeek 行也用厂商字标，字标随包分发**：与 Kimi 同一条构建通路（`src/assets/icons/wordmarks/deepseek.svg` 由 build.mjs 发现并内联成 `WORDMARK_SVGS`，按品牌 id 绑定）。harness 自己也画了一枚（`BrandWordmark`，官方侧栏品牌位渲染的就是它），但它是个**锁定标**而不是纯字标：`includeMark: false` 只裁掉前面的鲸鱼，后面那块圆角徽标仍在 viewBox 内，而徽标上写的是 HARNESS——模型行只要厂商名，所以改用随包的这一枚。资产裁到大写带：原文件 viewBox 是 `0 0 131 24`，墨迹是 x 2..128.73、y 0..23.456，其中基线在 19.404、大写顶在 0.649（「k」的上伸部到 0，「p」的下伸部到 23.456）；裁成 `2 0.649 126.73 18.755` 之后盒子底边就是基线，于是与 Kimi 共用同一条 `height: 1cap` 规则，上伸部与下伸部落在框外、靠 svg 的 `overflow: visible` 照常绘制（像文字的上伸下伸一样）。宽度按 cap 高算是 63.25px，而文字「DeepSeek」是 62.83px，几乎等宽。原文件的 `height="1em"`、`style` 与 `<title>` 一并去掉——`title` 会变成 hover 提示，无障碍由名称里的隐藏文本负责。
- **模型名里的厂商名改用厂商字标（Kimi 起）**：模型行自 0.2.7 起带厂商标识，但名称里的厂商名仍是普通文字——同一个「这是谁的模型」的信号，一半用标识、一半用正文字体。现在厂商若提供**字标**，名称里那个词就用厂商自己的字形绘制。触发条件是**供应商或模型名命中**该厂商，两条路径缺一不可：前者让 Kimi 官方目录里只叫 `K3` / `K3-256k` / `K2.8Preview` 的行也认得出（名称里没有那个词，字标就放在名称开头，并带走原词后面的分隔符），后者让 OpenCode 转售的「Kimi K3」不至于漏掉（词被替换，其余原样）。字标是 96×32 的单路径 SVG，与 Lobe 标识走同一条构建通路（`src/assets/icons/wordmarks/*.svg` 内联成 markup 表，按品牌 id 绑定）；高度取 `1cap`——正好等于标签字体的 cap 高（Anthropic Sans 量得 1440/2000，13px 下 9.36px），`0.72em` 是不支持 `cap` 单位的浏览器兜底（两条都带 `!important`，否则兜底会按顺序胜出）；宽度 28.08px 对文字「Kimi」的 26.79px，只宽 1.3px，所以行宽不跳。字标在 DOM 里是装饰性标记（`aria-hidden`），名称里另有一个视觉隐藏的厂商名，屏读听到的仍是「Kimi K3」而不是「K3」。

### Changed
- **厂商字标扫描改为预置数组，并补上分项计时探针**：`modelWordmark()` 原本每次调用都对词表做 `for...in` 加动态键查找、还把三个词重新 `toLowerCase()`——`.debug/wordmark-cost.cjs` 量出这段扫描在每行标签构建里约占 0.5µs（50 行一次渲染约 25ms）。现在词表在加载时构建成数组（小写形式与展示形式并列），每行只剩一次 `indexOf` 与一次比较；同一份量具复测，混合目录（3/10 命中字标）下整行成本从约 0.78µs 降到约 0.72µs，行为由 24 项夹具确认未变。同时新增 `scripts/probe-timing.cjs`：在真实浏览器里分项计时——启动长任务与插件资源、模型目录就绪（以触发器的模型名标签为观察点）、选择器打开延迟、行构成、字标 markup 的 `innerHTML` 解析成本、JS 堆——把「哪一段慢」从推测变成测量。
...
### Fixed
- **一级弹层「当前模型」那行的供应商、以及两级都显示模型描述**：三处相关调整。(1) 那行原先显示 `供应商id/模型id`，上一版按「外显名而非 id」改成了只显示模型名，把供应商整个丢了——现在恢复 `供应商/模型` 的格式，但两侧都用外显名（供应商取 `group.name`），**只有带字标的模型例外**：字标本身就是厂商名，再加前缀等于把厂商说两遍，所以那种行不显示供应商。(2) 「更多模型」是每个 provider 的完整目录，描述行在那里只是噪声，改为只在第一级显示。(3) 一级那行补上描述——它和官方列表里的行一样是「读」的行，描述正是把模型区分开的东西。
