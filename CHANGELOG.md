...
### Added
- **冒烟测试 `npm run smoke`（`scripts/smoke.cjs`）**：零依赖、不需要运行中的 dsh，直接检查构建产物。宿主半边：用假的 cordis 上下文驱动 `lib/index.js`，对私有路由发跨站页面、局域网、DNS 重绑定、浏览器同源、桌面壳转发几种请求，分别走宿主的 `requestRejection()` 与本地替身，核对拒绝与放行、415 / 413 / 400 与数组上限。浏览器半边：在无头 Chrome/Edge 里把 `lib/client.js` 装进一个替身宿主页面（模块加载器、ctx、带账号菜单与两个插件条目的侧栏页脚、按宿主键位处理回车的输入框），核对启动完整、空闲时 0 pass、设置 / 账号 / 插件来的字符串不按 markup 解析、回车交给宿主、宿主 API 出故障时只退役对应特性（安装期与同步期各一例）、teardown（包括 pass 正在排队时的 teardown）不留任何节点、标记、属性与样式表。另有一个 `desktop` 用例按 0.1.7 桌面端的真实页脚形状（账号菜单住在 `settings.launcher` 槽、设置区只剩一行 `triggerRow`）搭替身：这条路径此前完全没被覆盖——宿主那一行必须被接管隐藏、抽屉要把宿主菜单的 Settings / Feedback / Sign out 镜像出来而自己那行让位、从抽屉点设置与按 Ctrl+, 都要打开宿主设置且不留下打开着的账号菜单、登出图标必须留在自己的图标位里（修过一次的「图标跑到抽屉角落」回归）、账号状态流只在首帧读一次资料、同状态重复帧不再读。它第一次跑就抓到了两个此前没人发现的问题：对话页权限按钮让调度器空转，以及 teardown 之后复活的那一轮 pass。
- **账号行显示真实头像与昵称（桌面端登录）**：桌面端登录后，皮肤从官方 `remote.account.getProfile()` 取昵称（`value.name ?? contact`）与 `avatarUrl`，账号行改绘**正圆头像**（`border-radius: 50%`），照片加载失败时手绘星芒仍作为兜底显示；未登录 / 非桌面端 / 取数失败时保持原有的星芒与已存用户名。账号信息**只在启动、热重载、登录与登出时**向服务器读取（登录、登出无需刷新页面），不轮询。
...
- **模型名与档位离得太远、与后面的控件之间空隙过大**：档位触发器挂在 body 上由 JS 逐帧钉位（宿主 React 插槽排斥外来节点），定位常数是「模型按钮右缘 +8px」加按钮自身 10px 左 padding，两段文本隔了约 26px；为档位预留的右边距也比实际宽 14px，把上下文仪表 / 发送按钮推远。现在档位触发器压进模型按钮 8px 右 padding 里的 6px——盒子相叠、文本相距约 4px，读作一个词组（与当初 `margin-left: -6px` 草稿的意图一致，那段 CSS 自按钮改挂 body 起就不再命中）；右预留收紧为「宽度 −6 + 8px 余量」。换到无档位的模型、或特性退役时，预留的右边距也一并撤掉，不再留下死空隙。
