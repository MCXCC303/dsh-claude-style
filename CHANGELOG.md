...
- **侧栏「插件」行与新会话行对齐**：0.1.7 在新会话按钮下新增了一个 panel row（`插件`），宿主给它 36px 高 / 14px / 12px 圆角，而新会话行是 28px / 13px / 6px —— 两行叠在一起却像两份不同的列表。现在面板行取同一套度量（**含 `min-height`**：宿主用 min-height 把它顶在 36px，只写 `height` 压不住）、同一块底板与 hover、以及同一个 13px 图标盒（两行的文字因此从同一 x 起排）。实测两行 computed：`28px / 256px / x=12 / 6px / 13px 500 / padding 0 8px / gap 6px` 完全一致，图标盒都是 13×13。
