...
- `src/` — all source code: `constants.js` (constants and the 185 thinking verbs, evaluated at build time to fill tokens), `context/*.js` (host/prefs/model-copy/i18n), `overrides/*.js` (selection/composer/copy/permissions/model-brand/model-copy-lookup/model-picker/effort-picker/hero-menu/quick-providers/account-footer/ban-screen/theme-flip/workspace-view/view-tabs/scheduler + the shared popover-utils; extracted helper fragments: account-footer's profile/host-menu/rows/footer-mirror/surface live in the `account/` subdirectory, model-picker's catalog/rows live in the `model/` subdirectory, effort-picker's control/matrix live in the `effort/` subdirectory, permissions' session-stats.js sits next to the feature — split layout in docs/architecture.md D13), `settings.js` (settings-page brand switching), `entry.js` (the `apply()` orchestrator), `styles/**/*.css` (split by composer and components: composer/{hero,card,inline,inline-bar}, components/{permissions,account-footer,ban-screen,model-picker,effort-picker,popover,hero-menu,footer-takeover,third-party,settings,theme-flip}), `assets/brand/*.svg` (brand marks, inlined as data URIs at build time), `assets/icons/combine/*.svg` (vendor lockups: icon + vendor wordmark combined into one SVG, inlined at build time as a JS markup table), `assets/icons/*.svg` (hand-provided lockup assets, same level as the generated `combine/`, taking priority over network fetching during vendoring), `model-descriptions.json` (model copy data + the `brands` brand bindings and `brands.lockups` override table).
- `lib/` artifacts: `client.js` (generated), `index.js` (host side, handwritten, provides private routes like `/dsh-claude-style/model-descriptions.json`, security constraints in docs/architecture.md D11), `model-descriptions.json` (copied at build time), `claude-mark.svg` (copied at build time from `src/assets/brand/claude-mark-clay.svg` — this is `package.json`'s `icon`, which the 0.1.7 plugin manifest reads for the plugin card icon).
...
- CHANGELOG format (same spec as dsh upstream release notes):
  ...
  - Content policy: one entry per verifiable behavior or contract, no implementation diaries; changes to external contracts (settings, private routes, host version requirements) must be called out explicitly; performance changes carry locally measured numbers; user-facing behavior changes stay in sync with the bilingual READMEs and docs.
