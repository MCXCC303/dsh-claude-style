...
- Artifact boundary: build output rides in this history up to and including `1e36088` — that merge, plus the two source commits before it (`4b24add`, `a69106b`), were committed under the older rule that paired every source commit with its rebuilt `lib/`. Every source commit after `1e36088` carries none: `npm run build` refreshes the working tree only, and the next commit that touches `lib/` is the next release (or a dedicated `chore(build)` catch-up). `git log -1 -- lib/client.js` names the boundary at any time; those three artifacts are in sync with their own source, and no history was rewritten to remove them.
