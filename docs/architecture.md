...
- **背景**：HDSL 启动器用 `HDSL_` 前缀的环境变量向实例公布「这是谁」：账号显示名、供应商、账号种类、头像来源，以及玩家头像 PNG 的绝对路径；`HDSL_ACCOUNT_CONTRACT` 是唯一必需键，读不到它整组作废。浏览器半边读不到进程环境，头像又是启动器数据目录里的一张图，两者都只能由宿主半边转发。
- **决定**：
  - 宿主半边只信 `process` 与 `user-env` 两层（项目目录的 `.env` 会随仓库被克隆，没有资格声明玩家是谁），契约版本不等于 `1` 时回 `{ contract: false }`，其余字段一律不看。
  - 新增两条私有路由，与 `/username` 同级过 `refusalOf` 栅栏：`GET /dsh-claude-style/hdsl` 回账号元数据，**不含**头像文件的绝对路径；`GET /dsh-claude-style/hdsl-skin.png` 回头像字节——路径只来自环境，永远不来自请求，读不到就回 404 让浏览器回退。
  - 读环境走 harness 自带的 `@deepseek-ai/dsh-launch-environment`（`launchEnvironmentOf(ctx)`）。解析方式与 schemastery 相同：`createRequire(process.argv[1]).resolve()` 拿到真实路径后动态 `import`。这是 AGENTS 记录在案的第二条例外，理由与 D10 相同——`link:` 安装的插件真实路径在 profile 树之外，普通 import 解析不到，而解析不到时皮肤必须照常加载。
...
- **理由**：契约在进程生命周期内不变，读一次就够，所以路由只转发、不轮询；把两条链收在一处，欢迎语、账号行、封号彩蛋不会各自漂移；缓存探测到的系统用户名，是为了刷新页面时欢迎语不再从 `User` 跳一次。
- **代价**：桌面端宿主的账号行由宿主自己渲染，插件只在自建账号行上画头像，所以 HDSL 头像在桌面端只作用于插件自己的表面。
- **重审条件**：HDSL 契约递增版本、或宿主提供「显示名 / 头像」的统一服务时，改接那一处，删掉本插件的转发路由。
