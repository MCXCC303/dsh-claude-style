...
## D15. 启动器账号契约：只在宿主半边读，浏览器只拿显示所需

- **背景**：启动器（HDSL）启动实例时发布一组账号环境变量（`HDSL_ACCOUNT_*`，见其 `docs/plugin-guide/`）：账号名、供应商、账号种类、皮肤选择与一张规范化贴图的绝对路径。浏览器半边读不到进程环境，贴图也只有路径；而项目目录里的 `.env` 会随仓库一起被克隆，没有资格声明「玩家是谁」。启动器的皮肤是一张 64×64（或整数倍）的贴图集，不是头像：脸只是其中 (8,8) 起的 8×8 贴图块，帽子层在 (40,8)，渲染方式由插件决定。
- **决定**：契约只在宿主半边（`lib/index.js`）读一次，读的是 harness 自带的启动环境槽位 `ctx.launchEnvironment`（`getFrom(name, ['process', 'user-env'])`，省略的层不可达），因此不需要 import 启动器或 harness 的包，也不会被项目目录的 `.env` 影响；`HDSL_ACCOUNT_CONTRACT` 不是 `1` 时整组忽略。身份路由 `/dsh-claude-style/username` 回答「生效的名字」——启动器账号名优先，其次系统用户名——并把账号的显示字段（名字、供应商、种类、皮肤选择、有没有图）挂在同一份回答里；皮肤文件路径不出进程。贴图另开一条只读路由 `/dsh-claude-style/skin`：过宿主请求栅栏，只服务启动器点名的那一个文件，文件不存在或不是 PNG 一律 404。浏览器侧按启动器账号列表同样的裁法取头部（脸的 8×8 贴图块按盒子 1/18 内缩，帽子层铺满整盒），失败依次回退到宿主账号头像与手绘标记。
- **理由**：身份属于「只有宿主机进程才知道的事实」，与系统用户名同类，因此复用同一条既有的身份路由，而不是新增一条设置通道（D10 仍然只走 `configForms`，本条不改偏好读写）。名字不经过设置表单，所以设置服务在启动瞬间不可用也不会退回系统用户名（issue #6 的表现在此路径上消失）；读取用的是 harness 自己的层快照，项目目录无从冒充。裁脸几何与启动器账号列表一致，同一个玩家在启动器与皮肤里长得一样。
- **代价**：皮肤路由在每个插件里各写一份（启动器的 v2 计划里有按指纹取图的通道，届时改为优先走它）；只有玩家自选的本地图片能画，启动器内置的默认形象（steve/alex 等）没有像素可用，回退到手绘标记；契约在进程生命周期内不变，玩家中途换皮肤要下次启动才生效。
- **重审条件**：启动器实现 v2 的媒体通道（`HDSL_ACCOUNT_MEDIA` / `SKIN_SHA256`）时，取图改为优先走该通道、以指纹作缓存键；契约出现 UUID 时，按玩家存储的数据改用 UUID 作键。

## D16. 权限档位以宿主目录为准，皮肤只做表现层

- **背景**：权限控件（输入框分段控件与它的弹层）此前把四档写死在两张常量表里，只对内置 `auto`（Auto review）做一次目录判定。于是第三方插件注册的权限档位看不见也点不到——例如 auto mode 插件的 `auto-mode`（宿主 `permissionPresets` 目录里的一等条目：确定性规则 + 两阶段分类器），而且会话已经处于该档位时，触发键上会显示机器值 `auto-mode`。宿主的目录是「当前可选档位」的权威：`PermissionCatalog.options` 由宿主按贡献顺序给出，`PresetOption` 只有 `value` / `name` / `description`（部署打上 automode 提供的补丁或宿主将来原生支持时另有 `icon`），没有沙箱与审批策略。
- **决定**：段位与弹层行都按宿主目录构建——目录里有的档位才画，宿主没提供的整条不出现（不再有「画出来再隐藏」）；`PERMISSION_SEGMENTS` 只描述**格子**，每格列出可绑定的档位、按先后取第一个宿主提供的（部署自己的自动档优先于宿主内置的 Auto review），一格都绑不上就不画。档位名与说明来自皮肤的 `PERMISSION_PRESETS`，皮肤不认识的档位用目录自带的 `name`/`description`，永不把机器值上屏（宿主的 `custom` 用宿主自己的词）。行的内容是纯文字：档位声明的 `icon`（部署方写在预设上的 SVG 路径，宿主原生会忽略、由第三方补丁开启）**不画**——同一条列表里只有第三方档位带图标会读成两种东西，档位之间的区别交给名字与说明。切换仍走宿主的 `/permission <preset>`（与 automode 的 `/auto` 是同一条写入路径）；风险确认名单仍按 id 维护（完全权限与内置 Auto review），因为目录的 `PresetOption` 不带沙箱与审批策略，无法据此推导——第三方自动档把沙箱收进工作区、审批收成询问，属于收紧，不确认。
...
- **重审条件**：宿主在 `PresetOption` 里带上沙箱与审批策略时，风险确认改为按策略推导，删掉 id 名单；宿主原生支持预设 `icon`、且界面上有了统一的档位图形语言时，再考虑把图标画回来。
