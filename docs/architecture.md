...
- **决定**：
  ...
  - 头像不是直接贴上去的：启动器给的是贴图集，浏览器按启动器账号列表同样的裁法取头部（脸的 8×8 贴图块按盒子 1/18 内缩、帽子层铺满整盒）画进 64×64 画布再缩到账号位的尺寸，方形绘制不被圆角裁切；图取不到、契约没带图、文件被删时回退到官方头像与徽标。画完主动叫一次 pass：贴图加载不改 DOM，没有 pass 会自己来。
