...
6. **Account area** — after a desktop sign-in the row shows the **real avatar and nickname** (kept fresh by a 60-second poll); the account drawer mirrors the host's own menu (Settings / Feedback / Sign out — or Sign in), and each action calls the official behavior directly: the questionnaire opens in a new tab, sign-out hits the account namespace, and the host's own trigger stays out of sight. On hosts without the desktop account the drawer keeps only its Settings entry.
