...
3. **Model picker** — the composer's model seat becomes a two-level Claude-style menu: the first level lists DeepSeek's official models, a divider, the reasoning-effort row and "More models", and the second opens beside it. Model descriptions appear on the first level only (including the row for the current selection, which reads `provider/model` in display names); the "More models" level is every provider's full catalog and lists names alone. Every model row carries the mark of the **vendor that made the model** (not the provider reselling it), and the "More models" group headers carry the provider's mark. The marks are [Lobe Icons](https://lobehub.com/icons) mono glyphs painted in the theme's own text colour, so they stay legible on both canvases — no React dependency, and no full-colour logos. Where a vendor ships a **wordmark**, its own lettering replaces the vendor's name inside the label: a Kimi row draws the Kimi wordmark in place of the word, while a row the official catalog only calls `K3` / `K3-256k` has no such word and leads with the wordmark instead — the trigger is the provider *or* the model name matching the vendor, so a reseller's "Kimi K3" is recognised too. The wordmark follows the text colour as well, and hidden text keeps the full vendor name in the accessibility tree, so a screen reader still hears "Kimi K3" rather than "K3". DeepSeek rows work the same way (their wordmark is vendored as well: the harness's own artwork is a lockup that carries a HARNESS badge, and a row wants the vendor's name alone). Grok rows too (its wordmark is Lobe Icons' Grok text variant, and the mark on the row is Grok's own rather than xAI's).
