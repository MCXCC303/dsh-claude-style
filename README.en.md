...
6. **Account area** — after a desktop sign-in the row shows the **real avatar and nickname** (read from the server only at startup, on hot reload, and on sign-in and sign-out — never polled); the account drawer mirrors the host's own menu (Settings / Feedback / Sign out — or Sign in), and each action calls the official behavior directly: the questionnaire opens in a new tab, sign-out hits the account namespace, Settings opens the host's settings, and the host's own account row stays out of sight (along with the connection and update pills that share it). On hosts without the desktop account the drawer keeps only its Settings entry.
