...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "  /* Horizontally centred in the header, and on the title's own line. The host's",
      "     header rows are a 10px inset + a 30px title row (centre y=25) + a 10px margin",
      "     + the tab line, so the strip's natural top is y=50 and -38px lifts its centre",
      "     (26px tall) onto the title's centre. Measured on a header rebuilt from the",
      "     host's own classes: title cy=25, strip cy=25 with this. The strip already",
      "     spans grid-column 1/-1, so justify-self alone centres it. */",
