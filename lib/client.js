...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Claude artwork as alpha masks, taken from the official artwork (claude.ai
     * via Wikimedia Commons, public domain as a simple geometric/text logo). The
     * sidebar paints the starburst alone; its wordmark is typeset "Claude Code"
     * text (the official artwork has a "Claude" wordmark but no "Code" variant),
     * and the new-conversation hero uses the starburst with the clay fill baked in.
     *
     * Two details are load-bearing:
     *   - The source path lives in the artwork's own coordinate space and is
    ...
    var CSS = [
      ...
      "",
      "   Sizing: the marks sit on an 18px baseline to match the shipped brandName box.",
      "   Claude preset: the starburst is the official mask and the wordmark is typeset",
      "   \"Claude Code\" text — the artwork has a \"Claude\" wordmark but no \"Code\" variant,",
      "   so no mask can cover it. Anthropic preset wordmark keeps an 18px mask at its",
      "   natural aspect; both carry a max-width so a longer brand can never clip. */",
      ...
      "}",
      "",
      "/* Claude (default): the official starburst mark mask. */",
      ...
      "}",
      "",
      "/* Claude (default): official starburst mask + typeset \"Claude Code\" wordmark.",
      "   The text inherits the host brandName's own type style — no skin-side font",
      "   overrides, so it renders like the rest of the brand row. */",
