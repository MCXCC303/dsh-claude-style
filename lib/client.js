...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "  overflow: hidden !important;",
      "}",
      "/* The level's own trigger, beside the model's. Same plate and metrics, quieter",
      "   ink: the model is the seat's subject, the level is its modifier. */",
      "body[data-dsh-claude-style][data-dsh-claude-composer-active] [class*=\"trailing\"] .dsh-claude-effort-btn {",
      "  display: inline-flex !important;",
      "  align-items: center !important;",
      "  gap: 2px !important;",
      "  height: 28px !important;",
      ...
      "  min-width: 0 !important;",
      "}",
      "body[data-dsh-claude-style][data-dsh-claude-composer-active] [class*=\"trailing\"] .dsh-claude-effort-btn:hover {",
      "  background: var(--dsh-claude-hover-bg, rgba(0, 0, 0, 0.08)) !important;",
      "  color: var(--dsw-alias-label-primary) !important;",
      "}",
      "body[data-dsh-claude-style][data-dsh-claude-composer-active] [class*=\"trailing\"] .dsh-claude-effort-btn > * {",
      "  background: transparent !important;",
      "}",
      "body[data-dsh-claude-style][data-dsh-claude-composer-active] [class*=\"trailing\"] .dsh-claude-effort-btn-label {",
      "  text-overflow: ellipsis !important;",
      "  white-space: nowrap !important;",
      "  min-width: 0 !important;",
      "  overflow: hidden !important;",
    ...
    function createEffortControl(opts) {
      ...
      function ensureMatrix() {
        ...
        /* The plume: the nozzle is the RIGHT end (where the knob sits at the top
           rung) and the exhaust streams leftward. A block's flash is therefore
           delayed in proportion to its distance from the nozzle, so the light
           arrives as ONE front travelling right → left instead of a field of
           independent twinkles — which is what hash-scattered phases produced.
           The wave takes longer to cross the grid (TRAVEL_S) than a block stays
           lit (the stylesheet's 480ms light phase), so the front is a visible
           band: ahead of it the grid is dark, behind it the blocks fade back. */
        var TRAVEL_S = 1.0
        var step = TRAVEL_S / Math.max(1, lay.cols - 1)
        for (var r = 0; r < lay.rows; r++) {
          for (var c = 0; c < lay.cols; c++) {
            ...
            /* Tone follows the plume too: near the nozzle the particles flash at
               the brightest peak, and the exhaust cools as it travels left, with
               just enough per-block scatter that the tail is not banded. Only a
               BUCKET index is picked here — the colours themselves live in the
               stylesheet, so nothing can go stale in the DOM. */
            var blend = fx * 0.75 + cellUnit(r, c, 1) * 0.25
            sq.setAttribute('data-tone', String(Math.max(0, Math.min(3, Math.floor(blend * 4)))))
            /* The front is not a ruler-straight line — a real plume is ragged —
               but the scatter stays well under one column step, so it reads as
               one wave rather than as noise. */
            var delay = 0.2 + (1 - fx) * TRAVEL_S + (cellUnit(r, c, 2) - 0.5) * step * 0.7
            sq.style.setProperty('animation-delay', delay.toFixed(3) + 's', 'important')
            /* No per-block duration: the stylesheet's shared 1.45s cycle keeps
               the front coherent. A jittered duration (the earlier random
               twinkle) drifts the phases apart round by round and smears the
               wave back into noise after a couple of pulses. */
            cell.appendChild(sq)
