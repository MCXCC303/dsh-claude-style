      ...
      function openShippedGate(preset) {
        ...
        function seek() {
          ...
          for (var i = 0; i < items.length; i++) {
            ...
            var ariaLabel = (items[i].getAttribute('aria-label') || '').trim()
            for (var j = 0; j < targetLabels.length; j++) {
              var target = targetLabels[j]
              if (text === target || ariaLabel === target || text.indexOf(target) === 0 || ariaLabel.indexOf(target) === 0) {
                items[i].click()
                return
              }
            }
