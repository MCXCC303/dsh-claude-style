...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "  background: linear-gradient(to bottom, var(--dsw-alias-bg-base, #FCFCFB), transparent) !important;",
      ...
      "  background: linear-gradient(to top, var(--dsw-alias-bg-base), transparent) !important;",
      "  pointer-events: none !important;",
      "}",
      ...
      "   hands every block its own flash peak (`--dsh-flash-light`) — the flash is",
      "   a COLOUR change, from violet to a lighter violet, because a mere opacity dip",
      "   can only darken toward the track and the twinkle reads as blocks going",
      "   LIGHTER. The grid lives only while the apex is shown, so the repaint cost",
      "   of colour animations stays bounded to that state. */",
    ...
    function createEffortControl(opts) {
      ...
      function place(x, animate) {
        ...
        var width = steps.length === 0 ? '0px' : (Math.round(x) + 8) + 'px'
        if (animate || dragging) {
      ...
      function ensureMatrix() {
        ...
        /* Claude's Ultracode violet at rest, a lighter violet at the flash
           peak (both measured off the recorded slider). Every block mixes its
           own peak between the two (k in [0.45, 0.95)), so the grid twinkles in
           a range of tones instead of one shared light. The stylesheet's
           --dsh-claude-apex carries the matching rest colour. */
        var DEEP = [139, 122, 208]
        var LIGHT = [190, 182, 235]
        var PEAK = [219, 212, 246]
        var peak = []
        for (var pi = 0; pi < 3; pi++) peak.push(Math.round(LIGHT[pi] + (PEAK[pi] - LIGHT[pi]) * 0.3))
        for (var r = 0; r < lay.rows; r++) {
          for (var c = 0; c < lay.cols; c++) {
            ...
            var k = 0.45 + cellUnit(r, c, 1) * 0.5
            var lc = []
            for (var ci = 0; ci < 3; ci++) lc.push(Math.round(DEEP[ci] + (peak[ci] - DEEP[ci]) * k))
            sq.style.setProperty('--dsh-flash-light', 'rgb(' + lc[0] + ',' + lc[1] + ',' + lc[2] + ')')
      ...
      function settle() {
        ...
        paintValue()
        commit(index)
        /* paintValue above ran on the OLD selected; the commit moves it, so
           paint once more for the apex attribute to catch up. */
        paintValue()
      }
