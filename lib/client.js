...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "}",
      "/* A vendor wordmark stands in for the vendor's name inside a row's label: the",
      ...
      "   two agree exactly on the shipped face). Both declarations carry !important, or",
      "   the fallback would win on order. */",
      ...
      "}",
      "/* Each wordmark's viewBox is cropped to its lettering — DeepSeek's to its",
      "   capitals, so its box bottom edge is the baseline; Grok's to its ink, round \"G\"",
      "   overshoot and all. Whatever the art draws outside that box (DeepSeek's \"k\"",
      "   ascender and its \"p\" descender) still paints, because overflow stays visible,",
      "   exactly where a text ascender and descender would. */",
