...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "  right: 0 !important;",
      "  top: 50% !important;",
      "  /* Nearly as tall as the 16px knob (Claude's real Effort slider reads as one",
      "     chunky pill, not a bead on a wire): 6px read as a thin rail the knob sat",
      "     ON, 14px reads as a groove the knob rides IN. */",
      "  height: 14px !important;",
      ...
      "  transition: transform 0.18s cubic-bezier(0.22, 0.61, 0.36, 1) !important;",
      ...
      function ensureMatrix() {
        ...
        for (var r = 0; r < lay.rows; r++) {
          for (var c = 0; c < lay.cols; c++) {
            ...
            /* The flash dips the block's opacity onto the lighter substrate:
               an opacity animation composites, a background-colour one repaints
               every cell every frame. */
            sq.style.setProperty('--dsh-fo', fade.toFixed(3))
            sq.style.setProperty('--dsh-fo-min', (fade * 0.25).toFixed(3))
