 ...
 *
 * Recreates the Claude Code Desktop visual & interaction experience:
 *   - Ivory canvas #FCFCFB (light) / warm-black #141413 (dark), sidebar #FBFBF9
 *   - Clay accent #D97757, hover #C6613F
 *   - Serif display for headings, sans for UI chrome, mono for code
 *   - Plan | Edit | Auto segmented permission controls
 *   - Sidebar account button & popover drawer with settings (Ctrl+,)
 *   - Retracted cleanly via ctx.effect disposer on teardown
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Claude Code copy: the new-conversation headline and the composer hint. */
    var HERO_HEADLINE = 'Coffee and Claude time?'
    var COMPOSER_HINT = 'How can I help you today?'

    /**
     * Anthropic's starburst mark (claude-color.svg, #D97757) as a CSS image.
     * Inlined so the skin stays a single-file bundle with no asset request.
     */
    ...
    /**
     * Claude Code permission segments: the label shown, and the shipped preset
     * each one selects (plan = view only, edit = workspace write, auto = the
     * full-access preset that drops approval prompts).
     */
    var PERMISSION_SEGMENTS = [
      { label: 'Plan', preset: 'read-only' },
      { label: 'Edit', preset: 'workspace-write' },
      { label: 'Auto', preset: 'danger-full-access' },
    ]

    /** Preset the shipped UI gates behind its risk-confirmation dialog. */
    var GATED_PRESET = 'danger-full-access'
    /** Shipped full-access item labels, used to find that row in the shipped menu. */
    var FULL_ACCESS_LABELS = ['完全权限', 'Full access']
    /** Last-resort prompt, used only when the shipped menu cannot be reached. */
    ...
    var CSS = [
      ...
      '}',
      '',
      'body[data-dsh-claude-style]:not([data-ds-dark-theme]) :is([data-pane="sidebar"], [class*="sidebarCol"], .dshDesktopSidebarSurface, [class*="footArea"]) {',
      '  background: #fbfbf9 !important;',
      '  border-right-color: #e8e6dc !important;',
      '}',
      '',
      ...
      '   a Plan | Edit | Auto segmented control over the same three presets. */',
      ...
      '}',
      'body[data-dsh-claude-style] [class*="projectRow"][aria-expanded="true"] [class*="chevron"]::after {',
      '  transform: rotate(0deg);',
      '}',
      'body[data-dsh-claude-style] [class*="projectRow"][aria-expanded="false"] [class*="chevron"]::after {',
      '  transform: rotate(-90deg);',
      '}',
      '/* Visibility is owned entirely by the display rule above; an expanded folder',
      '   no longer pins its chevron open, and no opacity gate competes with it. */',
      'body[data-dsh-claude-style] [class*="projectRow"] [class*="chevron"] {',
      '  opacity: 1;',
      '}',
      '',
      ...
      '/* footArea positioning with crisp hairline border */',
      ...
      '  color: var(--dsw-alias-label-primary, #141413) !important;',
      '}',
      'body[data-dsh-claude-style][data-ds-dark-theme] .dsh-claude-popover-item:hover {',
    ...
    /** The shipped access-mode trigger, found by its localized aria-label. */
    ...
    /** The session the composer is bound to, or null when unavailable. */
    ...
    /** The session's current permission preset, or null when unavailable. */
    ...
    /** Build the segmented control once; its listener is bound for its lifetime. */
    ...
    /**
     * Install the Claude Code surface rewrites: the two copy slots (hero
     * headline, composer hint) and the permission segmented control. All of them
     * are React-rendered — the headline and hint re-render on locale switches and
     * the hint unmounts while a draft exists, while the composer re-renders the
     * access trigger on every preset change — so one MutationObserver re-applies
     * them after each render. Rewrites are idempotent (a node already in the
     * target state is left alone, and the control is only inserted when absent),
     * so the observer cannot feed itself.
     * @param ctx - owning context, read lazily for the sessions service.
     * @returns the observer teardown.
     */
    function installOverrides(ctx) {
      ...
      /** The hero headline span, skipping the preview badge beside it. */
      ...
      /** The visible composer hint, and only while it shows an idle hint. */
      ...
      /** Run the shipped /permission command for one preset. */
      ...
      /**
       * Drive the shipped access menu to its full-access row, so the switch runs
       * through the shipped risk-confirmation dialog rather than a skin-owned
       * prompt. The trigger is hidden by this skin but still in the tree, so a
       * synthetic click still opens the menu; the row is then picked by its
       * label. A plain confirm stands in only when that menu cannot be reached,
       * which keeps the switch behind an explicit acknowledgement either way.
       */
      function openShippedGate() {
        ...
        function seek() {
          ...
          trigger.click() // close the menu this attempt opened
          if (window.confirm(GATED_PROMPT)) submitPreset(GATED_PRESET)
        }
        requestAnimationFrame(seek)
      }

      /** Switch the composer's session to one preset. */
      ...
      /**
       * Mirror the shipped access control: keep one segmented control as the
       * trigger's first sibling, and mark the segment the session currently runs.
       */
        ...
        try {
          if (ctx && typeof ctx.get === 'function') {
            ...
            if (ws && ws.list && typeof ws.list.getSnapshot === 'function') {
              var items = ws.list.getSnapshot().items || []
              for (var i = 0; i < items.length; i++) {
                var p = items[i].path || ''
                var m = p.match(/[/\\](?:Users|home)[/\\]([^/\\]+)/i)
                if (m && m[1]) return m[1]
              }
            }
            var sessions = ctx.get('sessions')
      ...
      function syncPopoverItems(footArea) {
        ...
        // 2. Footer action buttons (e.g. 手机连接 / Cordis / Plugins)
      ...
      function syncAccountFooter() {
        ...
        // 1. Clean up any rogue popover items directly inside footArea (from past disposer runs)
        var rogues = footArea.querySelectorAll(':scope > .dsh-claude-popover-item')
        for (var r = 0; r < rogues.length; r++) {
          rogues[r].parentElement.removeChild(rogues[r])
        }

        // 2. Create account button if missing or detached
        ...
        // Clean up duplicate account buttons
        var allBtns = footArea.querySelectorAll(':scope > .dsh-claude-account-btn')
        for (var b = 0; b < allBtns.length; b++) {
          if (allBtns[b] !== accountBtn) {
            allBtns[b].parentElement.removeChild(allBtns[b])
          }
        }

        // 3. Create popover if missing or detached
        ...
        // Clean up duplicate popovers
        var allPops = footArea.querySelectorAll(':scope > .dsh-claude-account-popover')
        for (var p = 0; p < allPops.length; p++) {
          if (allPops[p] !== accountPopover) {
            allPops[p].parentElement.removeChild(allPops[p])
          }
        }

        // 4. Synchronize popover items
    ...
    /**
     * Apply the Claude Style skin: body attribute, style tag, and the Claude
     * Code surface overrides (copy slots + permission segments). All writes are
