...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "  /* `scale` is the drag lift (see data-dragging below): it is an INDEPENDENT",
      "     property, so it composes with the inline translateX the override writes",
      "     every frame instead of fighting it. The explicit 1 gives the release a",
      "     numeric target to ease back to (an unset value is `none`, which does not",
      "     interpolate). */",
      "  scale: 1 !important;",
      "  transition: transform 0.18s cubic-bezier(0.22, 0.61, 0.36, 1), scale 0.12s ease, background-color 0.3s ease, box-shadow 0.3s ease !important;",
