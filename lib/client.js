...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function installModelPicker(ctx, ui) {
       ...
       * is for. A provider counts as shown when its section is listed, and also
       * when it is the seat level 1 surfaces as a row of its own: its name is on
       * screen either way, and repeating it one card over is what made the same
       * models appear twice.
       */
      function remainingGroups(groups, sections, current) {
        var shown = {}
        for (var i = 0; i < sections.length; i++) shown[sections[i].id] = true
        if (current !== null && current !== void 0) shown[current.group.id] = true
        var out = []
      ...
      function renderModelBody() {
        ...
        if (!seated && (status === 'idle' || status === 'loading' || status === 'selecting')) {
          ...
          if (modelFooter) {
            ...
            if (effort !== null) modelFooter.appendChild(effortControlElement())
            ...
            if (remainingGroups(groups, sections, current).length > 0) {
      ...
      function renderModelSub() {
        ...
        var rest = remainingGroups(groups, sections, current)
