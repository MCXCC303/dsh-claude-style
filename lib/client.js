...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "   Guarded against the host's compact mode: there the host draws its own icon",
      "   readings with its own spacing, and there is no trigger or panel for the",
      "   merged sentence to stand in for, so the host's row is left untouched. */",
      ...
      "     the self-built drawer's transition becomes a one-shot entry animation with",
      "     the same values and the same origin (the drawer opens above the trigger). */",
      "  transform-origin: bottom left !important;",
      ...
      "  white-space: nowrap !important;",
      "  cursor: pointer !important;",
      "  /* Above the row's own layers (the card is 2, the stats sentence 3) and",
      "     nothing else: a page-level surface (the plugins page, a dialog) must be",
      "     free to cover it — and once the seat goes the trigger is hidden anyway. */",
      "  z-index: 4 !important;",
      "  transition: background-color 0.12s ease, color 0.12s ease !important;",
      "}",
