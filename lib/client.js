...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "  /* Tight against the model trigger: the two read as one phrase (\"Opus 5.5",
      "     High\"), so only the model's own 8px right padding survives, minus a 6px",
      "     pull-back, plus 2px here — a ~4px gap. Symmetric padding would have left",
      "     16px of dead air between the name and its level. */",
      ...
      "}",
      "/* The drawer's own Settings row steps aside where the host's account menu",
      "   carries Settings itself (the desktop). The shared popover-item rule sets",
      "   `display: flex !important`, which `hidden` alone cannot beat. */",
      "body[data-dsh-claude-style] .dsh-claude-account-popover .dsh-claude-popover-item[hidden] {",
      "  display: none !important;",
      "}",
      "/* Mirrored host icons keep the host's own geometry. The sign-out glyph is a",
      "   13.7px svg placed absolutely inside the host's own 16px relative box, and the",
      "   drawer copies the svg alone: without a containing block of its own the glyph",
      "   anchored to the drawer and sat in its top-left corner. The shared",
      "   popover-item rule also forces every icon svg to 16px, which would blow the",
      "   glyph up past its box, so host icons keep their own size (capped at 16px). */",
      "body[data-dsh-claude-style] .dsh-claude-account-popover .dsh-claude-popover-item-icon {",
      "  position: relative !important;",
      "}",
      "body[data-dsh-claude-style] .dsh-claude-account-popover [data-dsh-claude-account-item] .dsh-claude-popover-item-icon svg {",
      "  width: auto !important;",
      "  height: auto !important;",
      ...
      "   pinned beside the model trigger by view-tabs-style measurement on every pass. */",
      "body[data-dsh-claude-style] .dsh-claude-effort-btn {",
      "  position: fixed !important;",
      "  display: inline-flex !important;",
      "  align-items: center !important;",
      "  height: 28px !important;",
      "  /* Tight against the model trigger: positionEffortTrigger() already overlaps",
      "     6px of the model trigger's right padding, so the 2px here lands the two",
      "     texts ~4px apart — they read as one phrase (\"Opus 5.5 High\"). The 6px on",
      "     the right keeps the hover plate off the control that follows. */",
      ...
      "   The trigger row is not settingsArea's child: it renders inside the",
      "   `sidebar.settings` slot anchor (a display:contents div), which the child",
      "   selector alone never crossed. The row stayed up, squeezed into the zeroed",
      "   settingsArea, and what does not shrink painted over the skin's account row:",
      "   the connection pill, and on the desktop the update pill (\"Retry update\")",
      "   beside the host's own account trigger, which holds that slot there. The",
      "   account menu and its sign-in dialog are portals and the settings panel is",
      "   the row's sibling, so all three still open. */",
      "body[data-dsh-claude-style][data-dsh-claude-footer-takeover] [class*=\"footArea\"] > [class*=\"settingsArea\"] > [class*=\"triggerRow\"],",
      "body[data-dsh-claude-style][data-dsh-claude-footer-takeover] [class*=\"footArea\"] > [class*=\"settingsArea\"] > [data-slot=\"sidebar.settings\"] > [class*=\"triggerRow\"],",
