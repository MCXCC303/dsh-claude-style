...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "}",
      "",
      "/* A preset the host's permission catalog does not carry is not drawn at all:",
      "   src/overrides/permissions.js builds both the rows and the segments from that",
      "   catalog, so there is no hidden row to take out of the control. The rows carry",
      "   no glyph either — a third-party tier's declared `icon` included — so the",
      "   ladder reads as one list. */",
      "",
      ...
      "   a canvas cropped out of the texture the host serves (src/overrides/account/",
      "   rows.js), so it is the picture, not the mark, that fills the box. The mark is",
