...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "   inside, which is how Claude marks its primary \"New\" row. */",
      ...
      "   When the desktop has a real avatar, that mark has to go — otherwise both paint",
      "   and the star shows through. The picture rides a ::after layer with its URL in",
      "   a custom property, so the mark is removed in one place and the photo is drawn",
      "   in another, with no fight over `background-image`. */",
      ...
      "  display: none !important;",
      "}",
      "body[data-dsh-claude-style] .dsh-claude-account-avatar[data-dsh-claude-photo]::after {",
      "  content: \"\" !important;",
      "  position: absolute !important;",
      "  inset: 0 !important;",
      "  background: var(--dsh-claude-account-photo) center / cover no-repeat !important;",
      "  border-radius: inherit !important;",
      "  pointer-events: none !important;",
      "}",
      "",
      "/* While the skin drives the host's account menu (to read its items and to",
      "   forward a click), the menu must never be seen. Hiding, not removing, keeps the",
      "   items clickable: the drive dispatches its events directly at them. */",
      "body[data-dsh-claude-account-driving] [role=\"menu\"] {",
      "  visibility: hidden !important;",
      "}",
      "",
      ...
      "}",
      "",
      "/* ---------- Reasoning-effort slider (first level's footer) ---------- */",
      ...
      "   control reads as one phrase, `Reasoning effort Default`, the way the ends",
      "   below read as one scale. A real change replays a short blur-in (re-armed",
      "   from the override); mid-drag changes skip it and simply stay crisp. */",
      ...
      "}",
      "/* The ends name the axis, so they stay in English in every locale and in the",
      "   caption colour: they are a scale, not a value. */",
      ...
      "  background: color-mix(in srgb, var(--dsw-alias-label-primary, #141413) 26%, transparent) !important;",
      "}",
      "/* The top rung's dot matrix: a grid of violet blocks floating on the bare",
      "   track (the 1px gaps ARE the grid lines). The override solves the grid in",
      "   whole device pixels, pins each block's mask fade as a static opacity, and",
      "   picks each block's tone bucket (`data-tone`) — every COLOUR here lives in",
      "   this stylesheet and never inline, so a stylesheet swap can never leave a",
      "   stale palette behind. The flash is a colour change, from violet to a lighter",
      "   violet, because a mere opacity dip can only darken toward the track and the",
      "   twinkle reads as blocks going LIGHTER. The grid lives only while the apex is",
      "   shown, so the repaint cost of colour animations stays bounded to that state. */",
      ...
      "     top rung is actually reached, and re-showing replays the entrance sweep. */",
      "  display: none !important;",
      ...
      "     values. Four buckets span 45%→95% of the way to the light end, which is",
      "     the spread the twinkle needs without any block reaching the rest colour. */",
      ...
      "  --dsh-flash-light: color-mix(in srgb, var(--dsh-claude-apex, #8b7ad0) 55%, var(--dsh-claude-apex-flash, #d9d2f5));",
      "}",
      ...
      "  --dsh-flash-light: color-mix(in srgb, var(--dsh-claude-apex, #8b7ad0) 5%, var(--dsh-claude-apex-flash, #d9d2f5));",
      "}",
    ...
    function installModelPicker(ctx, ui) {
       ...
       */
      function layoutModelFooter(showEffort, showMore) {
        if (!modelFooter) return
        // Keep the slider when it is wanted; drop it (like everything else) when
        // the seat's model offers no levels.
        var keep = showEffort ? effortControlElement() : null
        var stale = []
        for (var child = modelFooter.firstChild; child !== null; child = child.nextSibling) {
          if (child !== keep) stale.push(child)
        }
        for (var i = 0; i < stale.length; i++) modelFooter.removeChild(stale[i])
        if (showEffort) {
          if (keep.parentNode !== modelFooter) modelFooter.appendChild(keep)
          modelFooter.insertBefore(modelEl('div', 'dsh-claude-model-divider'), keep)
        } else if (showMore) {
      ...
      function renderModelBody() {
        ...
        /* The footer is NOT cleared here: layoutModelFooter owns it, and it keeps
           the effort slider node attached. Clearing would detach the slider, and
           re-inserting it restarts every CSS animation inside it (the name's
           blur-in and the apex matrix's entrance sweep) — the control double-took
           on every host round-trip. The branch below lays the footer out either
           way, so nothing is left over.
        */
        ...
        if (!seated && (status === 'idle' || status === 'loading' || status === 'selecting')) {
          ...
          // Nothing to configure: drop the footer rows (the slider goes with them).
          layoutModelFooter(false, false)
        } else {
          ...
          if (modelFooter) {
            ...
            // under the list reads as a stray rule. layoutModelFooter owns that
            // judgement — and, crucially, keeps the slider node attached (see its
            // comment: re-inserting it restarts the control's animations).
            layoutModelFooter(showEffort, showMore)
          }
        }
        updateEffortControl()
      }
    ...
    loadAccount()
      ...
      /** The host's account trigger — the menu anchor carrying its own label. */
      function hostAccountTrigger() {
        var anchors = document.querySelectorAll('[aria-haspopup="menu"]')
        for (var i = 0; i < anchors.length; i++) {
          var el = anchors[i]
          if (el.className && String(el.className).indexOf('dsh-claude-') !== -1) continue
          var label = el.getAttribute('aria-label') || ''
          var rect = el.getBoundingClientRect()
          // The account trigger is the narrow sliver our own footer takeover left
          // behind, so "has a label and is not one of ours" is enough.
          if (label !== '' && rect.width > 0 && rect.height > 0) return el
        }
        return null
      }
      ...
      /**
       * Open the host's account menu invisibly, hand its items to `read`, then
       * close it again. `read` runs while the menu is still open.
       */
      function withHostAccountMenu(read) {
        ...
        for (var attempt = 0; attempt < 12; attempt++) {
          ...
          for (var i = 0; i < now.length; i++) {
            if (Array.prototype.indexOf.call(before, now[i]) === -1) menu = now[i]
          }
          if (menu !== null) {
            items = menu.querySelectorAll('[role="menuitem"]')
            if (items.length > 0) break
          }
          // A synchronous wait: the host renders the portal on the next tick, and
          // this half has no async plumbing inside the popover sync.
          var until = Date.now() + 40
          while (Date.now() < until) { /* spin */ }
        }
        if (menu !== null) menu.setAttribute(ACCOUNT_MENU_ATTR, '')
        if (read !== undefined) read(menu, items)
        realClick(trigger)
        document.body.removeAttribute(DRIVING_ATTR)
        return menu !== null
      }

      /** Read the host's items once, and re-read when the account state changes. */
      function refreshAccountItems() {
        var signature = (accountName || '') + '|' + (accountAvatar || '') + '|' + (getUsername(ctx) || '')
        if (signature === refreshAccountItems.last && accountItems.length > 0) return
        var read = false
        ...
        refreshAccountItems.last = signature
        accountMenuError = !read
      }
      ...
      function syncAccountMenuItems() {
        ...
        refreshAccountItems()
        var wanted = accountMenuError ? [] : accountItems
        ...
        for (var i = 0; i < wanted.length; i++) {
          (function (entry) {
            ...
            row.addEventListener('click', function (event) {
              ...
              closePopover()
              // Forward: find the item with the same label in the host's menu and
              // click it there, so the host owns what the option actually does.
              withHostAccountMenu(function (menu, items) {
      ...
      function syncPopoverItems(footArea) {
        ...
        if (hostTrigger !== null && hostTrigger.style.display !== 'none') hostTrigger.style.display = 'none'
