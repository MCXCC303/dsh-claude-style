...
/** Our own cache document version; 2 added the per-session hour histogram, 3 the per-day per-model map. */
...
export function createUsage(ctx) {
  ...
  function summarize(days, sessionCount, source, hours) {
    const list = [...days.entries()].map(([date, buckets]) => ({
      ...
      // The day's tokens per route, for the models chart. Absent when the source
      // has no route attribution at all (the cost-meter ledger).
