 ...
 * buckets, from the cheapest source that can answer.
 *
 * Two sources, in this order:
 *
 *   1. The cost-meter plugin's ledger (`$DSH_HOME/storages/cost-meter/ledger.json`),
 *      read ONLY. Its `days` map is already a per-day token roll-up, so when it
 *      exists and covers the newest session activity there is nothing left to
 *      compute. The file is never written: its owner rebuilds the whole document
...
/** Our own cache document version; 2 adds the per-session hour histogram. */
 ...
 * hour of day — the dashboard's peak-hour cell. A replaced sample leaves its
 * hour the same way it leaves its day.
 *
...
function foldSession(events) {
  ...
  const bump = (day, hour, buckets, sign) => {
    hours[hour] += sign
  ...
  for (const event of events) {
    ...
    if (replacing) bump(last.day, last.hour, last.buckets, -1)
    bump(day, hour, buckets, 1)
    last = { turn, step, buckets, day, hour }
  }
  return { days, hours }
}
...
function daysToObject(days) {
  const out = {}
  for (const [day, buckets] of days) out[day] = buckets
  return out
}
...
export function createUsage(ctx) {
  ...
  function readCache() {
    ...
    for (const [id, entry] of Object.entries(rawSessions)) {
      ...
      const hours = Array.isArray(entry.hours) && entry.hours.length === 24
        ? entry.hours.map((value) => (Number.isFinite(Number(value)) ? Number(value) : 0))
        : new Array(24).fill(0)
      sessions.set(id, { size: entry.size, mtimeMs: entry.mtimeMs, days: daysFromObject(entry.days), hours })
    }
    return sessions
  }
  ...
  function summarize(days, sessionCount, source, hours) {
    ...
    for (const buckets of days.values()) addBuckets(totals, buckets, 1)
    return {
      source,
      computedAt: Date.now(),
      days: list,
      ...
      // no hour dimension, so a ledger answer carries no histogram at all.
  ...
  async function compute() {
    ...
    if (ledgerDays !== null) {
      ...
      if (activityLast === '' || activityLast <= ledgerLast) {
        ...
        return { ...summarize(ledgerDays, sessionIds.size, 'cost-meter'), sessions: logs.length }
      }
    }
