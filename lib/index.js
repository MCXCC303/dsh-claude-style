...
/**
 * The `launchEnvironmentOf` function of the harness's own launch-environment
 * library, or null when it cannot be resolved.
 *
 * Same reason as {@link resolveSchemaFactory}: a plugin installed by link
 * resolves its realpath outside the profile tree, so a plain import of a
 * harness-provided library fails outright. The library is ESM, so the anchor
 * resolves the real file and this half imports that path.
 *
 * @returns the accessor, or null.
 */
async function resolveLaunchEnvironmentOf() {
  try {
    const { createRequire } = await import('node:module')
    const anchor = typeof process.argv[1] === 'string' && process.argv[1] !== '' ? process.argv[1] : process.execPath
    const resolved = createRequire(anchor).resolve('@deepseek-ai/dsh-launch-environment')
    const module = await import(pathToFileURL(resolved).href)
    if (typeof module?.launchEnvironmentOf === 'function') return module.launchEnvironmentOf
  } catch { /* the anchor carries no launch environment: try normal resolution */ }
  try {
    const module = await import('@deepseek-ai/dsh-launch-environment')
    return typeof module?.launchEnvironmentOf === 'function' ? module.launchEnvironmentOf : null
  } catch {
    return null
  }
}
...
function createHdslAccount(ctx) {
  ...
  const read = () => {
    ...
    reading = resolveLaunchEnvironmentOf().then((launchEnvironmentOf) => {
      if (launchEnvironmentOf === null) return { contract: false }
      let env
      try {
