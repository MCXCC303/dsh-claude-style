...
3. **模型选择器** —— 输入框的模型席位换成 Claude 风格的两级弹层：一级列出 DeepSeek 官方模型、分隔线、推理等级与「更多模型」，二级在右侧展开。模型描述只出现在第一级（含当前选中的那一行，格式为「模型 (供应商)」的外显名）；二级「更多模型」是各 provider 的完整目录，只列名称。每个模型行显示**所属厂商的锁定标**——图标与厂商字标合成的一件图形，直接替换名称里的厂商名（`[DeepSeek 鲸鱼+字标] V4.1-Flash`）；名称里没有厂商词的（Kimi 官方目录只叫 `K3`）则锁定标领在开头。「更多模型」的分组标题只显示 provider 名称，不带图标。图形取自 [Lobe Icons](https://lobehub.com/icons)（MIT），合成规则也用 Lobe 自己发布的那份（每个图标 `style.js` 里的 `TEXT_MULTIPLE`/`SPACE_MULTIPLE` 就是字标相对图标的大小与间距，`es/toc.js` 给品牌色），在构建前的 vendoring 阶段合成为 42 个厂商各一个 `<svg>`，不含 React 依赖。亮色画布用彩色原图，暖黑画布换成单色、跟随主题文字色——Lobe 自己的 React 组件也是这个规则，因为 `#000`（Grok、Kimi）或 `#F1F0E8`（Anthropic）这类品牌色在暖黑上会消失。字标要和它替换的词对得上：Claude 行画 Claude 的、Grok 行画 Grok 的，而不是各自母公司的。名称里另有视觉隐藏的厂商名，屏读听到的仍是「Kimi K3」而不是「K3」。
