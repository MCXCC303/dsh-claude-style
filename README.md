...
3. **模型选择器** —— 输入框的模型席位换成 Claude 风格的两级弹层：一级列出 DeepSeek 官方模型、分隔线、推理等级与「更多模型」，二级在右侧展开。模型描述只出现在第一级（含当前选中的那一行，格式为「模型 (供应商)」的外显名）；二级「更多模型」是各 provider 的完整目录，只列名称。每个模型行显示**所属厂商**的标识（不是转售该模型的 provider），「更多模型」的分组标题显示 provider 的标识。标识取自 [Lobe Icons](https://lobehub.com/icons) 的单色字形，跟随主题文字色绘制，因此亮暗两套画布都清晰；不引入 React 组件依赖，也不使用彩色 logo。厂商若提供**字标**（wordmark），名称里的厂商名就用厂商自己的字形绘制：Kimi 行把「Kimi」替换成 Kimi 字标，而官方目录里只叫 `K3` / `K3-256k` 的行没有这个词，就把字标放在名称开头——触发条件是**供应商或模型名命中**该厂商，所以转售方目录里的「Kimi K3」也认得出。字标同样跟随文字色，名称里另有视觉隐藏的厂商名，屏读听到的仍是「Kimi K3」而不是「K3」。DeepSeek 行同理，只是它那枚也随包分发（`src/assets/icons/wordmarks/`）——harness 自绘的那枚是带 HARNESS 徽标的锁定标，模型行只要厂商名。Grok 行同样（字标取自 Lobe Icons 的 Grok 文字变体；它的行内标记是 Grok 自己的，不是 xAI 的）。
