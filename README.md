...
8. **首页版面** —— 新会话页有两套版面，在设置页随时切换：**经典**是居中大标题加输入卡片；**工作台**把标题移到左上并固定为一句「What's up next, 用户名？」，输入框改用对话内的单行样式贴住窗口底部、上方一行细边框上下文胶囊，标题与一块 480px 窄栏用量面板贴着输入框左缘排布：概览页签是六个数字格（会话数、调用次数、Token、活跃天数、高峰时段、常用模型）与 26 周按天热力图（等分列方格），模型页签按模型列出份额条，右上角的「全部 / 30 天 / 7 天」切换统计范围；累计用量超过一本《霍比特人》时，热力图下方出现倍数趣味行。数据由插件的宿主半边汇总：装了 `dsh-cost-meter` 时直接读它的账本缓存（只读，不写对方文件），没有时读本机会话日志自己算，并按日志的文件大小与修改时间增量缓存；模型数据与兜底数字取自宿主会话列表每行自带的投影值。数字没到之前面板先画骨架，位置在填充前后不变。
